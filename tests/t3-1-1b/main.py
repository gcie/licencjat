...
torch.manual_seed(397313885)
np.random.seed(246159523)
...
history = SPDG(model, optimizer_primal, optimizer_dual, sequence_loader,
               ...
               test_every, eval_predictions_on_sequences=True, show_dual=False)
...
ys = [[100.0 - preds[i, i] / preds[i].sum() * 100 for preds in history['predictions_sequences']] for i in range(model.output_size)]
mpl.rc('savefig', format='svg')
mpl.rc('lines', linewidth=0.5)
mpl.style.use('seaborn')
for i in range(model.output_size):
...
plt.xlabel('Epoch')
plt.ylabel('Error (%)')
plt.savefig("predictions_sequences_error")
plt.close()
