...
def SPDG(model, optimizer_primal, optimizer_dual, sequence_loader, data_loader, test_loader, num_epochs=5, log_every=1, test_every=1,
         ...
         ngram_data_stats=False, ngram_test_stats=False, loss_on_test=False, remember_dual=False):
    if history is None:
        history = History()
        if remember_dual:
            if model.ngram.size() > 20:
                warnings.warn(f"Model\'s n-gram is large ({model.ngram.size()} entries). Remebering dual variables \
        ...
        while epoch_ < num_epochs:
            ...
            if epoch_ % test_every == 0:
                ...
                if predictions_on_sequences:
                    ...
                    if ngram_data_stats:
                        predictions, ngram_data = get_statistics(model, data_loader=sequence_loader, sequences=True, return_ngram=True)
