...
def get_sequences_distribution(sequence_loader):
    stats = defaultdict(int)
    for _, y in sequence_loader:
        for sample in y.to('cpu').numpy():
            for x in sample:
                stats[x] += 1
    return dict(stats)
...
def show_stats(stats):
    ...
    for k in sorted(stats):
        print('{:d}: {:.1f}% ({:d} total)'.format(k, 100 * stats[k] / total, stats[k]))
...
def get_statistics(model, data_loader=None, sequences=False, return_ngram=False):
